...
AS
BEGIN
	INSERT INTO D_DE_DATOS.CUPONES_DESCUENTO(cod_cupon,cod_usuario,monto_cupon,fecha_alta_cupon,fecha_vencimiento_cupon,cod_tipo_cupon)
	SELECT 
		M.CUPON_NRO,
		U.cod_usuario,
		M.CUPON_MONTO,
...
AS
BEGIN
    INSERT INTO D_DE_DATOS.PEDIDOS_CUPON(cod_cupon, cod_pedido)
...
BEGIN
	...
	(
		...
		LEFT JOIN gd_esquema.Maestra M2 ON M2.CUPON_NRO = M.CUPON_RECLAMO_NRO
		WHERE M.CUPON_RECLAMO_NRO IS NOT NULL AND M2.CUPON_NRO IS NULL
	)
...
--EXECUTE D_DE_DATOS.MigrarReclamosCuponesDescuento;
